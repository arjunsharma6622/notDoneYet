...
const CurrentConversation = ({currentConversation, setCurrentConversation, session, writeNewMsg, newUserToSendMsg, allFollowingUsers, setNewUserToSendMsg, setWriteNewMsg} : any) => {
    ...
    const handleSendMessage = async () => {
      ...
      const handleFirstMessage = async () => {
        try {
          const firstConversation = await axios.post(
            `/api/conversation/`, {
              senderId: session?.user._id,
    ...
    {currentConversation && (
      <div className=" flex items-start h-full gap-4">
        ...
        <div className="flex items-start gap-2 border-b px-4 py-3">
                <div>
                  <img src={currentConversation?.users?.filter((user : any) => user._id !== session?.user._id)[0]?.image} alt="" className="rounded-full h-10 w-10 object-cover" referrerPolicy="no-referrer"/>
                </div>
                <div className="text-sm flex flex-col">
                  <span>{currentConversation?.users?.filter((user : any) => user._id !== session?.user._id)[0]?.name}</span>
                  <span className="text-gray-400 text-xs">{currentConversation?.users?.filter((user : any) => user._id !== session?.user._id)[0]?.bio}</span>
               </div>
              </div>
          ...
          <div className="flex flex-col gap-4 w-full px-8 overflow-y-scroll h-full" >
            ...
            {currentConversation?.messages?.map((message : any, index : any) => {
              ...
              if(new Date(message.createdAt).getDate() !== new Date(currentConversation?.messages[index - 1]?.createdAt).getDate()) {
                return (
                  <div
                    key={index}
...
<div ref={messagesEndRef} />
        ...
        <div className="flex flex-col gap-4">
          {
            allFollowingUsers.map((user : any, index : number) => (
              <div key={index} className="flex items-start gap-2 cursor-pointer" onClick={() => {setNewUserToSendMsg(user); setWriteNewMsg(false); setCurrentConversation(null)}}>
                <div>
                  <img src={user.image} alt="" className="rounded-full h-10 w-10 object-cover" referrerPolicy="no-referrer"/>
                </div>
                <div className="text-sm flex flex-col">
                  <span>{user.name}</span>
    ...
    {newUserToSendMsg && (
                      ...
                      <div className="flex items-start gap-2 border-b px-4 py-3">
                <div>
                  <img src={newUserToSendMsg.image} alt="" className="rounded-full h-10 w-10 object-cover" referrerPolicy="no-referrer"/>
                </div>
                <div className="text-sm flex flex-col">
                  <span>{newUserToSendMsg.name}</span>
                  <span className="text-gray-400 text-xs">{newUserToSendMsg.bio}</span>
               </div>
              </div>

              <div className="flex flex-col gap-8 w-full h-full px-8 overflow-y-scroll">
