    ...
    <div className="relative flex items-center justify-center px-2 ">
      <div className="md:w-[95%] flex  md:flex-row flex-col gap-5 items-start mt-5">
            ...
            {session && userData?.role !== "user" ? (
              <>
                {userData?.role === "athlete" && (
                  <Athlete userData={userData} />
                )}
                {userData?.role === "venueOwner" && (
                  <VenueOwner userData={userData} />
                )}
                {userData?.role === "brand" && <Brand userData={userData} />}
                {userData?.role === "doctor" && <Doctor userData={userData} />}
              </>
            ) : (
        ...
        <div className="w-full flex md:flex-col flex-col md:flex-[4] gap-4">
          {userData && <RecommendedUsers userData={userData} />}
        </div>
      </div>
    </div>
  );
