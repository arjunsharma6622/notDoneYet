...
const AboutProfileEdit = ({ user, open, setOpen } : { user: any, open: boolean, setOpen: (open: boolean) => void }) => {
          ...
          <div className="w-[95%] md:w-[55%] max-h-[90%] bg-white rounded-md flex flex-col gap-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 overflow-scroll">
              ...
              <div className="flex items-center justify-end gap-4 border-t px-6 py-4">
                <Button variant="destructive" className="px-6  py-2 rounded-sm font-semibold" onClick={() => setOpen(false)}>
                  Cancel
                </Button>
                <Button type="submit" className="px-6 bg-primary py-2 rounded-sm font-semibold">
                  Save
                </Button>
              </div>
            </form>
