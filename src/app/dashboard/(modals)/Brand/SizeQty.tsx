  ...
  const handleSizeClick = (size: string) => {
    if (sizes.includes(size)) {
      ...
      setStock((prevStock: { size: string; quantity: number }[]) =>
        prevStock.filter((item) => item.size !== size),
      );
    } else {
          ...
          {productSizes[category].map((size, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              >
                ...
                <input type="text" value={size} className="hidden" readOnly />
                <label className="text-center cursor-pointer gap-2 w-10 h-10 px-2 py-2 rounded-md">
                  {size}
                </label>
              </div>
              {sizes.includes(size) && (
