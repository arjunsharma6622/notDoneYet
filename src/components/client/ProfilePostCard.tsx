...
const ProfilePostCard = ({ post, currUser, dashboardCard }: any) => {
  return (
    <div
      className="rounded-md flex flex-col gap-2 border px-2 py-2 w-full"
    >
      <div className="flex items-start flex-col justify-start gap-4 w-full">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex flex-col -gap-1">
            ...
            <span className="text-xs">{post?.user?.followers?.length} followers</span>
          </div>
          </div>

          <PostCardMore dashboardCard={dashboardCard} postData={post} currUser={currUser} />
