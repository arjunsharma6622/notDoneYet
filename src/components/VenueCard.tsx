    ...
    <div className="shadow-md border flex flex-col gap-3 rounded-lg px-5 py-4">
      ...
      <div className="overflow-x-scroll justify-start items-center gap-4 flex w-[calc(100%-10px)]">
        {venueDetails.images.map((image : any, index : number) => (
          <img key={index} src={image} className="rounded-lg w-56 h-44 object-cover" />
        ))}
      </div>
      ...
      <div>
        ...
        <button onClick={() => setOpenBookingModal(true)} className="bg-primary rounded-md text-sm px-5 py-2 text-white">
          Book Now
        </button>
...

      {
        <div className="absolut">
          <VenueBookingModal venueDetails={venueDetails} setOpenBookingModal={setOpenBookingModal} openBookingModal={openBookingModal} />
        </div>
      }
